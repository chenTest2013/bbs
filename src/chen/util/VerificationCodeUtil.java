...
System.out.println("第一种验证");
		Random random = new Random(); // 生成随机类
		Graphics g = initImage(image, random);
		String sRand = "";
		for (int i = 0; i < 4; i++) {//4 :产生4位数字
	...
	public void initChinesePlusNumVerificationCode(BufferedImage image) {
		System.out.println("第三种验证");
		String[] cn = { "壹", "贰", "叁", "肆", "伍", "陆", "柒", "捌", "玖", "拾","一", "二", "三", "四", "五", "六", "七", "八", "九", "十" };
		Random random = new Random(); // 生成随机类
		Graphics g = initImage(image, random);
	...
	public Graphics initImage(BufferedImage image, Random random) {
		...
		for (int i = 0; i < 165; i++) {// 随机产生165条干扰线，使图象中的认证码不易被其它程序探测到
